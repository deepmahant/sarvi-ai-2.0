...
import { User, Lock, ArrowRight, Eye, EyeOff, Facebook } from 'lucide-react';
import { getSupabaseRedirectUrl, supabase, trackUserSessionEvent } from '../../lib/supabase';
...
export function LoginForm({ onSuccess, className = "" }: LoginFormProps) {
  ...
  useEffect(() => {
    ...
    const rawErrorDesc = urlParams.get('error_description') || hashParams.get('error_description') || urlParams.get('error');
    if (rawErrorDesc) {
      const decoded = decodeURIComponent(rawErrorDesc.replace(/\+/g, ' '));
      if (decoded.toLowerCase().includes('invalid app id') || decoded.toLowerCase().includes('app id')) {
        setError('Invalid Facebook App ID: Please enter your real Facebook App ID & App Secret in Supabase Dashboard (Authentication -> Providers -> Facebook).');
      } else {
        setError(decoded);
      }
    }
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      if (isSignUp) {
        ...
        await trackUserSessionEvent({
          userEmail: normalizedEmail,
          userName: name || normalizedEmail.split("@")[0] || "User",
          eventType: 'login',
        });
      ...
      const userName = metadata?.full_name || data.user.email?.split("@")[0] || "User";
  ...
  const handleSocialSignIn = async (provider: 'google' | 'facebook') => {
    ...
    try {
      const options: Record<string, unknown> = {
        redirectTo: getSupabaseRedirectUrl('/chat'),
      };
      ...
      throw new Error(`Unable to start ${provider === 'google' ? 'Google' : 'Facebook'} sign-in.`);
    } catch (err: any) {
