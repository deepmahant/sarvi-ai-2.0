...
import { supabase, trackUserSessionEvent } from './lib/supabase';
...
function getAppUserFromSupabase(user: { email?: string | null; user_metadata?: Record<string, any> }): AppUser {
  ...
  return {
    name,
    email,
    role: email === 'admin@sarvi.ai' ? 'admin' : 'user',
  };
}
...
export default function App() {
  ...
  const handleLogout = async () => {
    try {
      ...
      // Ignore sign-out error, still clear local state.
    }
    localStorage.removeItem('sarvi_current_user');
    setUserState({ view: 'landing', user: null });
